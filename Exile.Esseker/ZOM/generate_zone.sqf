/*
EXILE-Z Project v3.1B - Edit for EXILE by SAM and CaptionJack of RelentlessServers.com
...
if (zA2Building == 1) then {_buildings = nearestObjects[_position,["Land_House_L_1_EP1"], 150];
	...
	for "_x" from 0 to _number do {
		...
		_civilrole = "civil";
		_back = [[_civiltype, _position, _civilrole]];
		//_back = _back + [[_civiltype, _position, _civilrole]]; //old code
	};
		...
		if(([(list _trigger)] call ZOMcountplayer == 0) or _start) then {
			_start = false;
			{
				if(alive _x) then {
					...
					_back = [[typeof _x, position _x, _civilrole]];
					//_back = _back + [[typeof _x, position _x, _civilrole]]; // old code					
					_x setDamage 1;
					deletevehicle _x;
				};
				sleep 0.1;
